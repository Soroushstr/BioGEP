"""
loo_pooled_v8.py — LOO cross-species evaluation with supervised contrastive loss

Changes vs v7:
  - SupCon loss (lambda_con=0.1, τ=0.1): explicitly aligns essential-gene
    representations across species so the backbone learns universal features
    rather than species-specific shortcuts.
  - Stronger GRL (lambda_adv=0.5 vs 0.2): species adversarial signal is louder.
  - More dropout (0.4 vs 0.3): extra regularisation to fight multi-species overfitting.
  - Contrastive projection head (proj_dim=128) is a separate 2-layer MLP so the
    classification head is unaffected.

Everything else (Bayesian prior correction, focal loss, 80/20 random val split,
species-balanced sampling, cosine LR, early stopping patience=30) is unchanged.

Usage:
  python loo_pooled_v8.py --held_out arabidopsis
  python loo_pooled_v8.py --held_out elegans
  ...

Submit all 8 in parallel:
  bash submit_all_v8.sh
"""
...
MODELS_DIR = os.path.join(SRC, "loo_models_v8")
os.makedirs(MODELS_DIR, exist_ok=True)

K           = 4
HIDDEN_DIM  = 256
NUM_LAYERS  = 4
DROPOUT     = 0.4      # v7=0.3 → more regularisation
EPOCHS      = 200
BATCH_SIZE  = 128
LR          = 3e-4
ES_PATIENCE = 30
LAMBDA_ADV  = 0.5      # v7=0.2 → stronger species-adversarial signal
LAMBDA_CON  = 0.1      # NEW: supervised contrastive loss weight
CON_TEMP    = 0.1      # SupCon temperature
...
print(f"Setting          : v8 — GRL + SupCon + Focal + Bayesian prior correction")
print(f"  lambda_adv     : {LAMBDA_ADV}  (v7 was 0.2)")
print(f"  lambda_con     : {LAMBDA_CON}  (NEW supervised contrastive loss)")
print(f"  con_temperature: {CON_TEMP}")
print(f"  proj_dim       : {PROJ_DIM}")
print(f"  dropout        : {DROPOUT}  (v7 was 0.3)")
...
model_path = os.path.join(MODELS_DIR, f"loo_model_{HELD_OUT}_v8.pt")
...
csv_path = os.path.join(OUT, f"loo_v8_{HELD_OUT}_results.csv")
