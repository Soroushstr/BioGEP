"""
new_pipeline_copy_v11.py — Phylogeny-aware species weighting

Two modes driven by loo_pooled_v11.py:

v11a (hard exclusion, no GRL)
  Training pool is pre-filtered to phylogenetically close species only.
  num_species=0 → GRL head disabled.  species_weights=None.

v11b (soft weighting, GRL kept)
  All 7 training species included.  species_weights={species_id: float}
  scales each sample's loss by exp(-beta * phylo_dist(species, held_out)).
  GRL kept — adversarial signal still present.

Everything else inherited from v10:
  - constant grl_alpha = 1.0
  - AUC-based early stopping
  - Bayesian prior correction in test()
  - cosine LR scheduler, focal loss, species-balanced sampling
"""
...
# ---------------------------------------------------------------------------
# Focal loss — supports reduction='none' for per-sample species weighting
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Bayesian prior correction (unchanged from v4/v10)
# ---------------------------------------------------------------------------
...
def train(
        ...
        # v11: per-species loss weight {species_id (int): weight (float)}
        # None = no weighting (identical to v10 behaviour)
    ...
    print(f"Early stopping metric: {early_stop_metric.upper()}  (v11: constant GRL alpha=1.0)")
    if species_weights is not None:
        print(f"Species loss weights: {species_weights}")
    ...
    if focal_loss_gamma > 0.0:
        ...
        criterion_none = FocalLoss(alpha=class_weights, gamma=focal_loss_gamma, reduction="none")
        print(f"Using Focal Loss (gamma={focal_loss_gamma:.1f})")
    else:
        criterion      = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=label_smoothing)
        criterion_none = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=label_smoothing,
                                             reduction="none")
    ...
    for epoch in range(1, epoch_n + 1):
        ...
        grl_alpha = 1.0  # constant (v10/v11 fix)
        ...
        for batch in train_loader:
            ...
            if use_sp_weights and hasattr(batch, 'species_id'):
                # Per-sample species weighting
                sw = torch.tensor(
                    [species_weights.get(int(sid), 1.0) for sid in batch.species_id.view(-1)],
                    dtype=torch.float, device=device,
                )
                per_loss = criterion_none(out, batch.y)
                loss = (per_loss * sw).mean()
                if use_adv and species_out is not None:
                    adv_per = F.cross_entropy(
                        species_out, batch.species_id.view(-1), reduction="none"
                    )
                    loss = loss + lambda_adv * (adv_per * sw).mean()
                ...
                if use_adv and species_out is not None and hasattr(batch, 'species_id'):
                    loss = loss + lambda_adv * F.cross_entropy(
